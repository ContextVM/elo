...
export function createJavaScriptBinding(): StdLib<string> {
  ...
  jsLib.register(
    "sum",
    [Types.array],
    (args, ctx) => `${ctx.emit(args[0])}.reduce((a, b) => a + b, 0)`,
  );
  jsLib.register(
    "avg",
    [Types.array],
  ...
  for (const t of [Types.string, Types.any]) {
    ...
    jsLib.register(
      "reverse",
      [t],
      (args, ctx) => `${wrapReceiver(args, ctx)}.split('').reverse().join('')`,
    );
  }
  ...
  jsLib.register("Interval", [Types.interval], (args, ctx) =>
    ctx.emit(args[0]),
  );
  ...
  jsLib.register(
    ...
    (args, ctx) => `${ctx.emit(args[0])}.union(${ctx.emit(args[1])})`,
  );
  jsLib.register(
    "intersection",
    [Types.interval, Types.interval],
  ...
  jsLib.register(
    "Duration",
    [Types.interval],
    (args, ctx) => `${ctx.emit(args[0])}.toDuration()`,
  );
